...
const CreateIncome = ({ open, onClose }: CreateIncomeProps) => {
  ...
  const {
    ...
    remarks,
    finance_category_id,
  } = useSelector((state: RootState) => state.createIncome);
  ...
  const fetchDropdownData = async () => {
    ...
    try {
      ...
      if (categoriesResponse.success && categoriesResponse.data) {
        ...
        console.warn('[CreateIncome] Categories response invalid:', { success: categoriesResponse.success, hasData: !!categoriesResponse.data });
      }

      if (accountsResponse.success && accountsResponse.data) {
        setBankAccounts(accountsResponse.data);
      } else {
        console.warn('[CreateIncome] Bank accounts response invalid:', { success: accountsResponse.success, hasData: !!accountsResponse.data });
      }
    } catch (error: any) {
      console.error('[CreateIncome] Error fetching dropdown data:', {
        error,
  ...
  const handleChange = (field: keyof CreateIncomeState, value: string) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await createIncomeApi(payload);

      if (result.success) {
        toast.success(result.message || "Income created successfully", {
          position: "top-center",
          autoClose: 3000,
        });
        dispatch(resetForm());
